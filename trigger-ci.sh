...
them. Unless specified differently, all jobs are run on master.
...
$ ./trigger-ci.sh -f JOB_NAME:some_job
...
for VAL in "${VARS[@]}"; do
    IFS=':' read -ra VAR <<< "$VAL"
    if [ "${#VAR[@]}" -eq 2 ]; then
        FORM_VARS="${FORM_VARS}-F variables[${VAR[0]}]=${VAR[1]} "
    else
...
RESP=$(curl -X POST -F token="$TRIGGER_TOKEN" -F ref="$REF" $FORM_VARS "$API_URL/projects/$PROJECT_ID/trigger/pipeline")

if [ 0 -eq $? ]; then
    STATUS=$(grep -Po '(?<="status":")[^"]*' <<< "$RESP")
    if [ "$STATUS" = "pending" ]; then
        echo -e "\nSuccess, pipeline is pending.\n"
    else
